...
MAPS = [
    # PRD v12: 맵 보너스 축소 (최대 +5/-3, 최대 스윙 8pt)
    # (name, terran_bonus(기동대), zerg_bonus(공세대), protoss_bonus(수호대))
    ("번개 평원",      +5, -3, +1),   # 기동대 특화
    ("습지 요새",      -1, +5, +1),   # 공세대 강점  (구: -2,+6,+1)
    ("금단의 땅",      +3, +2, -2),   # 기동+공세
    ("고대 신전",      +1, -3, +5),   # 수호대 특화  (구: +1,-4,+7)
    ("폐허 신전",       0, +4, +3),   # 공세+수호
    ("전방고지",       -2, +3, +5),   # 공세/수호 특화 (구: -3,+3,+5)
    ("전초기지",       +4, +1, -2),   # 기동대 특화
    ("폭풍지대",       +1, -3, +5),   # 수호대 강점  (구: +2,-5,+6)
    ("달빛초원",       -1, +5, +2),   # 공세대 강점
    ("격전지",         +2, +2, +2),   # 균형 맵      (구: +3,+2,+2)
]

# 구 맵명 → 신 맵명 매핑 (DB 마이그레이션용)
MAP_RENAME = {
    "블리츠":      "번개 평원",
    "파이썬":      "습지 요새",
    "네오 포비든존": "금단의 땅",
...
ITEMS = [
    # ── 단일 능력치 (+5) ──────────────────────────────
    # 컨트롤·공격력: OVR 가중치 0.20 → 190G (프리미엄)
    # 수비력·전략·물량·센스: OVR 가중치 0.15 → 130G (할인)
    ("컨트롤 강화제",  "컨트롤 +5",              190, "능력치",   5, 0, 0, 0, 0, 0, 0,  0),
    ("공격력 강화제",  "공격력 +5",              190, "능력치",   0, 5, 0, 0, 0, 0, 0,  0),
    ("수비력 강화제",  "수비력 +5",              130, "능력치",   0, 0, 5, 0, 0, 0, 0,  0),
    ("전략 매뉴얼",    "전략 +5",                130, "능력치",   0, 0, 0, 0, 5, 0, 0,  0),
    ("물량 부스터",    "물량 +5",                130, "능력치",   0, 0, 0, 5, 0, 0, 0,  0),
    ("센스 훈련서",    "센스 +5",                130, "능력치",   0, 0, 0, 0, 0, 5, 0,  0),
    # ── 복합 능력치 ───────────────────────────────────
    ...
    # ── 프리미엄 ──────────────────────────────────────
    ("정예 훈련서",    "공격력·컨트롤·전략 +3",   280, "능력치",   3, 3, 0, 0, 3, 0, 0,  0),
    ("올스탯 결정체",  "전 능력치 +3",            450, "능력치",   3, 3, 3, 3, 3, 3, 0,  0),
]
...
def seed():
    ...
    # ── 맵 이름 마이그레이션 (구 SC 맵명 → 오리지널명) ──────
    for old_name, new_name in MAP_RENAME.items():
        cur.execute("UPDATE maps SET name=? WHERE name=?", (new_name, old_name))

    # ── 맵 보너스 마이그레이션 (PRD v12: 최대 ±5/-3) ─────────
    for name, tb, zb, pb in MAPS:
        cur.execute(
            "UPDATE maps SET terran_bonus=?, zerg_bonus=?, protoss_bonus=? WHERE name=?",
            (tb, zb, pb, name)
        )
