...
MAPS = [
    # PRD v11: 모든 맵 이름을 오리지널로 교체 (IP 완전 제거)
    # (name, terran_bonus(기동대), zerg_bonus(공세대), protoss_bonus(수호대))
    ("번개 평원",      +5, -3, +2),   # 기동대 특화 (이전: 블리츠)
    ("습지 요새",      -2, +6, +1),   # 공세대 강점 (이전: 파이썬)
    ("금단의 땅",      +3, +2, -2),   # 기동+공세 (이전: 네오 포비든존)
    ("고대 신전",      +1, -4, +7),   # 수호대 특화 (이전: 아즈텍)
    ("폐허 신전",       0, +4, +3),   # 공세+수호 (이전: 로스트 템플)
    ("전방고지",       -3, +3, +5),   # 공세/수호 특화 (이전: 데스티네이션)
    ("전초기지",       +4, +1, -2),   # 기동대 특화 (이전: 스타게이트)
    ("폭풍지대",       +2, -5, +6),   # 수호대 강점 (이전: 블루스톰)
    ("달빛초원",       -1, +5, +2),   # 공세대 강점 (이전: 루나)
    ("격전지",         +3, +2, +2),   # 균형 맵 (이전: 투혼)
]
